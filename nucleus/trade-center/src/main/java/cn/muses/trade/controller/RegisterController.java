...
import static cn.muses.trade.constant.SysConstant.*;
import static cn.muses.trade.constant.SysConstant.EMAIL_REG_CODE_PREFIX;
...
//        return "registeredResult";
    ...
    @ResponseBody
    public MessageResult registerByEmail(@Valid LoginByEmail loginByEmail) throws Exception {
...
//        Object code =valueOperations.get(SysConstant.EMAIL_REG_CODE_PREFIX + email);
        Object code =redisUtil.get(EMAIL_BIND_CODE_PREFIX+email);
        System.out.println("code = " + code);
        System.out.println("loginByEmail.getCode() = " + loginByEmail.getCode());
        System.out.println("code.toString().equals(loginByEmail.getCode()) = " + code.toString().equals(loginByEmail.getCode()));
        ...
        //随机长度的盐
        String credentialsSalt = Md5.randSalts();
        //生成密码
        String password = Md5.md5Digest(loginByEmail.getPassword() + credentialsSalt).toLowerCase();

        //二次加密将md5加密成security支持验证的加密方式
...
//    }
    ...
    @ResponseBody
    @Transactional(rollbackFor = Exception.class)
    public MessageResult sendBindEmail(String email) {
