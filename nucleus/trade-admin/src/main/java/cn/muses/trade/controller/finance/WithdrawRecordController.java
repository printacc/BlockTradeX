...
import cn.muses.trade.es.ESUtils;
import cn.muses.trade.model.screen.WithdrawRecordScreen;
import cn.muses.trade.service.*;
import cn.muses.trade.util.MessageResult;
//import cn.muses.trade.vendor.provider.SMSProvider;
import cn.muses.trade.vo.WithdrawRecordVO;
...
import static cn.muses.trade.constant.BooleanEnum.IS_FALSE;
import static cn.muses.trade.constant.WithdrawStatus.*;
import static cn.muses.trade.constant.WithdrawStatus.FAIL;
import static cn.muses.trade.constant.WithdrawStatus.SUCCESS;
import static cn.muses.trade.constant.WithdrawStatus.WAITING;
...
//    private SMSProvider smsProvider;
    ...
    public MessageResult all() {
        ...
        return success(withdrawRecordList.toString());
    }
    ...
    public MessageResult detail(@PathVariable("id") Long id) {
        WithdrawRecord withdrawRecord = withdrawRecordService.getOne(id);
        notNull(withdrawRecord, messageSource.getMessage("NO_DATA"));
        return success(withdrawRecord.toString());
    }
    ...
    public MessageResult auditPass(@RequestParam("ids") Long[] ids) {
        ...
        for(Long id : ids) {
        	WithdrawRecord record = withdrawRecordService.getOne(id);
...
//					smsProvider.sendCustomMessage(member.getMobilePhone(), "尊敬的用户，万分抱歉，您于" + date + "提取 " + record.getCoin().getUnit() + "失败，数量为" + record.getArrivedAmount() + "，请联系客服处理。");
    ...
    public MessageResult remittance(
        ...
        for (Long id : ids) {
            withdrawRecord = withdrawRecordService.getOne(id);
