...
import cn.muses.trade.entity.QWithdraw;
import cn.muses.trade.entity.Withdraw;
import cn.muses.trade.model.screen.WithdrawScreen;
...
import cn.muses.trade.service.WithdrawService;
...
public class WithDrawController extends BaseAdminController {
    ...
    public MessageResult merge(@Valid Withdraw withdraw, BindingResult bindingResult) {
        ...
        Withdraw one = withdrawService.getOne(withdraw.getId());
        if (one == null) {
            result = error("请选择审核的记录");
            return result;
        }
