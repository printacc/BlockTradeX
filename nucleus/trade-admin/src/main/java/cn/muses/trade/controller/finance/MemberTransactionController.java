...
import cn.muses.trade.entity.QMemberTransaction;
import cn.muses.trade.es.ESUtils;
import cn.muses.trade.model.screen.MemberTransactionScreen;
import cn.muses.trade.model.vo.MemberTransaction2ESVO;
import cn.muses.trade.model.vo.MemberTransactionExcelVO;
import cn.muses.trade.model.vo.MemberTransactionFeeExcelVO;
...
import cn.muses.trade.vo.MemberTransactionVO;
...
public class MemberTransactionController extends BaseAdminController {
    ...
    public MessageResult all() {
        ...
        if (memberTransactionList != null && memberTransactionList.size() > 0) {
            return success(memberTransactionList.toString());
        }
    ...
    public MessageResult detail(@RequestParam(value = "id") Long id) {
        MemberTransaction memberTransaction = memberTransactionService.getOne(id);
        notNull(memberTransaction, "validate id!");
        return success(memberTransaction.toString());
    }
    ...
    public MessageResult getPageQueryByES( MemberTransaction2ESVO transactionVO) {
        ...
        try {
            ...
            query+="]}}}";
            return success(esUtils.queryForAnyOne(JSONObject.parseObject(query),"member_transaction","mem_transaction").toString());
        }catch (Exception e){
            log.info(">>>>>>查询异常>>>"+e);
            return error("查询异常");
        }
